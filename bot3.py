...
def agrobot(color,b):
    ...
    if color:
        location = [chess.square_file(b.king(chess.BLACK)),chess.square_rank(b.king(chess.BLACK))]
    else:
        location = [chess.square_file(b.king(chess.WHITE)),chess.square_rank(b.king(chess.WHITE))]

    evaluation = 0
    for x in range(8):
        for y in range(8):
            if x != location[0] and y != location[1]:
                evaluation += key[str(b.piece_at(chess.square(x,y)))]/((((location[0]-x)**2)+((location[1]-y)**2))**0.5)
    return evaluation
...
def bot(color, b, depth, lastnum=0, alpha=-1000000, beta=1000000):
    ...
    if depth < search_depth - 1:
        ...
        return agrobot(color, copy.deepcopy(b))+(20*calculate_material(copy.deepcopy(b)))
